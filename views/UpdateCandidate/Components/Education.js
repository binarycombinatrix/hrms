...
const Education = ({
  ...
  useEffect(() => {
    if (education.education && education.course) {
      setEducationForm(education.education);
      let withArrayOfSkills = education.course.map((course) => {
        let finalArray = course.skills
          .split(",")
          .map((item, itemIdx) => ({ id: itemIdx, title: item }));

        return { ...course, skillsArray: finalArray };
      });
      setCoursesForm(withArrayOfSkills);

      let tempEducationError = [];
      for (let i = 0; i < education.education.length; i++) {
        tempEducationError.push(initEducationError);
      }

      setEducationError(tempEducationError);

      let tempCourseError = [];
      for (let i = 0; i < education.course.length; i++) {
        tempCourseError.push(initCourseError);
      }
      setCourseError(tempCourseError);
    }
  }, [education]);
  useEffect(() => {
  ...
  const checkDuplicate = () => {
    let flag = true;
    let educationType = educationForm.map((item) =>
      parseInt(item.educationTypeId)
    );
    let course = educationForm.map((item) => parseInt(item.courseId));
    let passingYear = educationForm.map((item) => parseInt(item.passingYear));
      ...
      <Card>
        ...
        <CardBody>
          {educationAddError.inValid && (
            <Label className="text-danger">{educationAddError.errorMsg}</Label>
          )}
        ...
        <CardBody>
          {courseAddError.inValid && (
            <Label className="text-danger">{courseAddError.errorMsg}</Label>
          )}
            ...
            <>
              <div className="form-row" key={courseIndex}>
                ...
                <FormGroup className="col-md-6">
                  <Label for="exampleSelectMulti">Skills*</Label>
                  <Multiselect
                    options={skills}
                  ...
                  <Input
                    ...
                    multiple
                    onChange={(e) => {
                      handleCoursechange(e, courseIndex);
                    }}
                    invalid={courseError[courseIndex]["skills"].inValid}
                  >
                    <option
                      selected={course.skills.split(",").includes("html")}
                    >
                      html
                    </option>
                    <option selected={course.skills.split(",").includes("css")}>
                      css
                    </option>
                    <option selected={course.skills.split(",").includes("js")}>
                      js
                    </option>
                    <option
                      selected={course.skills.split(",").includes("nodejs")}
                    >
                      nodejs
                    </option>
                    <option
                      selected={course.skills.split(",").includes("mongodb")}
                    >
                      mongodb
                    </option>
                  </Input>
