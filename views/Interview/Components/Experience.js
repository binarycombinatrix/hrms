...
const Experience = ({
  ...
  const [designationData, setDasignationData] = useState([]);
  ...
  useEffect(() => {
    const getMaster = async () => {
      const resOne = await axios.get("/designation");
      const resTwo = await axios.get("/technology");
      if (resOne.success === true && resTwo.success === true) {
        setDasignationData(resOne.data);
        setTechnologyData(resTwo.data);
      }
    };
    getMaster();
  }, []);
                ...
                <div className="form-row" key={experienceIndex}>
                  ...
                  <FormGroup className="col-md-4">
                    ...
                    >
                      ...
                      {designationData.map((designation, idx) => (
