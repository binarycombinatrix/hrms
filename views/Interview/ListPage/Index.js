...
const Index = () => {
  ...
  const handleCandidateProfileRoute = (id) => {
    let encodedId = CryptoJS.AES.encrypt(JSON.stringify(id), "Yeah*/")
      .toString()
      .replaceAll("/", "xMl3Jk");
    History.push(`candidate/profile/${encodedId}`);
  };
  ...
  const search = async () => {
    ...
    const res = await axios.get(`/candidate`, { params });
    if (res.status) {
            ...
            <div className="form-row">
              ...
              <FormGroup className="col-md-4">
                ...
                <CustomInput
                  ...
                  value={searchParams.candidateStatusId}
                  onChange={handleSearchParams}
                  className="filter-drop"
                >
                  <option value="">Select candidate status</option>
                  <option value="5">interviewed</option>
                  <option value="6">lead</option>
                  <option value="7">blacklist</option>
                  <option value="8">employee</option>
                  <option value="9">NLC</option>
                  <option value="10">got the job</option>
                </CustomInput>
                <button
                  type="button"
                  className="close filter-clear"
                  onClick={() => close("candidateStatusId")}
                >
                  <span>×</span>
                </button>
              </FormGroup>
                    ...
                    {candidateList.length > 0 &&
                      candidateList.map((candidate, cmpIdx) => (
                        <tr key={cmpIdx}>
                          <td
                            onClick={() =>
                              handleCandidateProfileRoute(candidate.id)
                            }
                          ...
                          <td>{candidate.activity.candidateStatus.name}</td>
